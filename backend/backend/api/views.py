...
def check_filter(filter_is_favorite, is_in_shopping_cart, filter_author, filter_tags, recipe, shopping_list,
...
class CreateRecipe(ListAPIView, APIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        filter_is_favorite, is_in_shopping_cart, filter_author, filter_tags = filters(request=request)
        ...
        for el in update_data.get('ingredients', []):
            ...
            if float(el.get('amount')) < 1:
                return Response(status=status.HTTP_400_BAD_REQUEST,
                                data={'amount': 'Убедитесь, что это значение больше либо равно 1.'})
...
class DownloadShoppingList(APIView):
    ...
    def get_txt(self, data, name):
        with open(name, 'w', encoding='utf-8') as f:
            for el, value in data.items():
                f.write(f"{el} ({value['measurement_unit']}) - ({data[el]['amount']})\n")
